...
class DPOptimizer(Optimizer):
    ...
    def __init__(
        ...
        *,
        ...
        self.compute_percentage_clipping = compute_percentage_clipping
        if self.compute_percentage_clipping:
            self.frac_clipped_samples = -1
    ...
    def clip_and_accumulate(self):
        ...
        #if per_sample_norms > 1 then fraction is < 1, and will be multiplied by grad_sample to reduce its norm to 1
        #if per_sample_norms <= 1 then the fraction is > 1, and it will be cut to 1 so that when multiplied by grad_sample
        ...
        if self.compute_percentage_clipping:
            self.frac_clipped_samples = per_sample_clip_factor
